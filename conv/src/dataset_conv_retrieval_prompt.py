...
class CRSConvDataset(Dataset):
    def __init__(
        ...
        # 使用 prompt_tokenizer 的 mask token 作为占位
        ...
        data_file = os.path.join(dataset_dir, f'{split}_data_processed_retrieval_bm25.jsonl')
    ...
    def prepare_data(self, data_file):
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                # -------- 新增：保留字符串形式的上下文（不含目标回复），供 F-former 使用 --------
                context_str = context
                # ----------------------------------------------------------------
                ...
                # demonstration 的 prompt（未使用拼接到输入，仅保留结构一致性）
                ...
                data = {
                    ...
                    'resp': resp_ids,
                    'entity': list(
                        dialog['retrieved_response_entity'] + dialog['retrieved_context_entity'] + dialog['entity']
                    )[-self.entity_max_length:],
                    'retrieved_example_ids': retrieved_example_ids,
                    'prompt': prompt_ids,
                    'retrieved_example_gen_ids': retrieved_gen_example_ids,
                    'context_str': context_str,  # ★ 新增：给 F-former 的原始上下文字符串
                }
    ...
    def __call__(self, data_batch):
        ...
        if self.gen:
            ...
            for data in data_batch:
                ...
                context_str_list.append(data['context_str'])  # ★ 新增
        else:
        ...
        # ★ 新增：把原始字符串上下文放进 batch（list[str]），供 F-former 使用
...
if __name__ == '__main__':
    ...
    for i in range(len(dataset)):
        ...
        print(data.keys())
        print('--- context_str ---')
        print(data['context_str'][:160].replace('\n', ' '), '...')
        print('--- context ids decode ---')
    ...
    data_collator = CRSConvDataCollator(
        ...
        prompt_tokenizer=prompt_tokenizer, gen=gen
    )
    ...
    for batch in tqdm(dataloader):
        if debug:
            ...
            print('context_str example:', batch['context_str'][0][:160], '...')
            break
