...
class CRSConvDataset(Dataset):
    def __init__(
        ...
        self.prompt_token = self.prompt_tokenizer.mask_token  # use tokenizer's mask token

        # Max lengths for different tokenizers/fields
        ...
        # Alternative retrieval-based data file:
    ...
    def prepare_data(self, data_file):
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                # Skip if no recommendations or empty context
                if len(dialog['rec']) == 0:
                    continue
                if len(dialog['context']) == 1 and dialog['context'][0] == '':
                    continue

                # Build the main conversation context
                context = '<D>:'
                prompt_context = ''
                ...
                # Build retrieved demonstration examples
                retrieved_examples = []
                ...
                for c, r in list(zip(dialog['retrieved_contexts'], dialog['un_mask_retrieved_resp']))[:self.n_examples]:
                    ...
                    for utt in list_utts:
                        ...
                        if i % 2 == 0:
                            ...
                            text1 += 'System: '
                            prompt_text1 += 'System: '
                        text1 += utt + self.tokenizer.eos_token
                        prompt_text1 += utt + self.prompt_tokenizer.sep_token
                ...
                for i, sent in enumerate(retrieved_examples):
                    ...
                    ids = self.prompt_tokenizer.convert_tokens_to_ids(
                        self.prompt_tokenizer.tokenize(self.prompt_token)
                    ) * self.prompt_max_length + ids
                ...
                # Ensure fixed number of retrieved examples
                ...
                # Prepare context and prompt IDs
                resp = ''  # masked response placeholder
                ...
                for item_id, item in list(zip(dialog['rec'], dialog['rec_movie_names'])):
                    ...
                    data = {
                        ...
                        'resp': resp_ids,
                        'entity': list(
                            dialog['retrieved_response_entity'] +
                            dialog['retrieved_context_entity'] +
                            dialog['entity']
...
class CRSConvDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        # Generation mode: left padding, context followed by "Item:" prompt
        if self.gen:
            ...
            # Training mode: right padding, concatenate context and resp
        ...
        if not self.gen:
            ...
            resp_batch = [[token_id if token_id != self.tokenizer.pad_token_id else -100 for token_id in resp]
