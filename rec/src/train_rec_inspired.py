...
from dataset_rec_inspired import CRSRecDataset, CRSRecDataCollator
...
def parse_args():
    ...
    parser.add_argument("--seed", type=int, default=64, help="A seed for reproducible training.")
...
if __name__ == '__main__':
    ...
    prompt_encoder = KGPrompt(
        model.config.hidden_size, text_encoder.config.hidden_size, model.config.num_attention_heads, model.config.num_hidden_layers, 2,
    ...
    for epoch in range(args.num_train_epochs):
        ...
        prompt_encoder.update_curriculum(epoch + 1)

        for step, batch in enumerate(train_dataloader):
            with torch.no_grad():
                token_embeds = text_encoder(**batch['prompt']).last_hidden_state
            prompt_embeds, Qloss = prompt_encoder(
            ...
            loss = model(**batch['context'], rec=True).rec_loss / args.gradient_accumulation_steps + 0.2 * Qloss
        ...
        epoch_model_dir = os.path.join(args.output_dir, f'epoch_{epoch}')
        os.makedirs(epoch_model_dir, exist_ok=True)
        prompt_encoder.save(epoch_model_dir)
        logger.info(f'Saved model for epoch {epoch} at {epoch_model_dir}')
        ...
        for batch in tqdm(valid_dataloader):
            with torch.no_grad():
                token_embeds = text_encoder(**batch['prompt']).last_hidden_state
                prompt_embeds, _ = prompt_encoder(
                    entity_ids=batch['entity'],
                    token_embeds=token_embeds,
                    context_str=batch['context_str'],
                    rec_labels=batch['context']['rec_labels'],
