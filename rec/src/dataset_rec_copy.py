...
class CRSRecDataset(Dataset):
    def __init__(
        ...
        # data_file = os.path.join(dataset_dir, f'{split}_recommend_processed_data_Redial.pkl')
        data_file = os.path.join(dataset_dir, f'{split}_recommend_processed_data_Redial_retrieval_random.jsonl')
        self.data = []
        # self.bert_tokenizer = AutoTokenizer.from_pretrained('/root/autodl-tmp/DCRS-main/rec/src/models/bert-base-uncased')
    ...
    def prepare_data(self, data_file):
        with open(data_file, 'r') as f:
            ...
            for line in tqdm(lines):
                ...
                text = ' '.join(dialog['context_tokens'])
                resp = ' '.join(dialog['response_word'])
                # context = self.bert_tokenizer.decode(self.bert_tokenizer.convert_tokens_to_ids(dialog['context_tokens']))
                # prompt_context = self.bert_tokenizer.decode(self.bert_tokenizer.convert_tokens_to_ids(dialog['context_tokens']))
                context = text
                prompt_context = text
                # try:
                #     resp = dialog['retrieved_resp'][0]
                # except:
                #     resp = ''
                ...
                ### filtering out most requent entities.
                count = defaultdict(int)
                for en in dialog['retrieved_response_entity'] + dialog['retrieved_context_entity']:
                    count[en] +=1
                ...
                data = {
                    ...
                    'prompt': prompt_ids,
                    # 'entity': [],
                    'entity': (dialog['context_entities'] + entitylist )[-self.entity_max_length:],
                    'rec': dialog['items'],
                    'context_str': context
                }
...
class CRSRecDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        for data in data_batch:
            ...
            context_str.append(data['context_str'])
        ...
        input_batch['context_str'] = context_str
